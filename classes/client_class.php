...
class client{
	...
	public function get_exercise($e_id){
		foreach($this->conn->query("SELECT e_title FROM exercise WHERE e_id = '".$e_id."'") as $row){
			return $row['e_title'];
		}
		
	}
	
	public function get_routine_title($ro_id){
		foreach($this->conn->query("SELECT ro_title FROM routine WHERE ro_id = '".$ro_id."'") as $row){
			return $row['ro_title'];
		}
		
	}
	
	public function count_ro_exs($ro_id){
		$sql = "SELECT e_id FROM routine_exercise WHERE ro_id = '".$ro_id."'";
		$query = $this->conn->prepare($sql);
		$query->execute();
		$count = $query->rowCount();
		return $count;
	...
	public function display_program_input($id){
		$i = 1;
		foreach($this->conn->query("SELECT cprog_routine_link.ro_id, cprog_routine_link.cprog_id FROM client_program INNER JOIN cprog_routine_link ON client_program.cprog_id = cprog_routine_link.cprog_id WHERE client_program.c_id = '".$id."'") as $row){
			
			echo '<h2>Routine '.$i.' - '.$this->get_routine_title($row['ro_id']).'</h2>';
			echo '<button type="button" class="cpp_ro_btn">Workout input</button>';
			echo '	<div class="cpp_ro_div">
						<form method="POST" action="index.php?p=client&c=sessions">';
				foreach($this->conn->query("SELECT e_id, sets, reps, reps_type FROM routine_exercise WHERE ro_id = '".$row['ro_id']."'") as $ex){
					echo '<label style="width:25%;display:inline-block;">'.$this->get_exercise($ex['e_id']).' <em>X '.$ex['sets'].' Set(s)</em></label>';

					//Check what type of ex it is 
					if($ex['reps_type'] == 'Mins' || $ex['reps_type'] == 'Metres'){
						$weight = '<input type="hidden" name="cprog_ex_w[]" value="0"/>';
					}
					else{
						$weight = '<input style="width:5%;" type="text" name="cprog_ex_w[]"/>
									<label>KG</label>';
					}
					
					
					
					$input_str = '	<div style="display:inline;border-right:1px solid #9a9a9a">
									<input style="width:5%;" type="text" name="cprog_ex_reps[]"/>
									<label>'.$ex['reps_type'].'</label>
									'.$weight.'
									
									</div>
									';
					echo str_repeat($input_str, $ex['sets']);
					echo '<hr>';	
					echo '	<input type="hidden" name="ex_id[]" value="'.$ex['e_id'].'"/>';
					echo '	<input type="hidden" name="cprog_ex_type[]" value="'.$ex['reps_type'].'"/>';
					echo '	<input type="hidden" name="cprog_ex_sets[]" value="'.$ex['sets'].'"/>';
						
				}
			
			echo '<input type="hidden" name="cprog_id" value="'.$row['cprog_id'].'"/>';
			echo '<input type="hidden" name="c_id" value="'.$id.'"/>';
			echo '<input type="hidden" name="ro_id" value="'.$row['ro_id'].'"/>';
			echo '<input type="submit" name="cprog_submit" value="Submit"/>';
			echo '</form>';
			echo	'</div>';
			echo '<hr style="margin-left:0.2em;margin-right:0.2em;">';		
			$i++;
		}
		
	}
	
	public function progress_insert($post, $c_id){
		$a = 0;
		$ret = '';
		$z = 0;
		foreach($post['cprog_ex_sets'] as $value => $row){
			$i = 0;
			$curr_date = date("Y-m-d H:i:s");
			while($i < $row)
			{	
			$b = $a + $i;
				if(empty($post['cprog_ex_reps'][$b])){
					$post['cprog_ex_reps'][$b] = 0;
				}
				if(empty($post['cprog_ex_w'][$b])){
					$post['cprog_ex_w'][$b] = 0;
				}
							
				$sql = "INSERT INTO client_program_progress (cprog_id, ro_id, e_id, reps_achieved, w_d_achieved, e_type, cpp_date) VALUES 
				(:cprog_id, :ro_id, :e_id, :reps_achieved, :w_d_achieved, :e_type, :cpp_date)";
				
				$query = $this->conn->prepare($sql);
					$query->bindParam(":cprog_id", $post['cprog_id']);
					$query->bindParam(":ro_id", $post['ro_id']);
					$query->bindParam("e_id", $post['ex_id'][$z]);
					$query->bindParam(":reps_achieved", $post['cprog_ex_reps'][$b]);
					$query->bindParam(":w_d_achieved", $post['cprog_ex_w'][$b]);
					$query->bindParam(":e_type", $post['cprog_ex_type'][$value]);
					$query->bindParam(":cpp_date", $curr_date); 
				$query->execute();	
				$i++;	
			}
		...
		return '<h2 style="background-color:#99FF66;padding:0.25em;">Your progress was inserted</h2></br>';
	}
